...
## 🔗 link_cert / unlink_cert / list_certs — 憑證關聯

### list_certs — 列出憑證
```bash
curl -s -X POST "https://fotan.techforliving.net/api/skill" \
  -H "Content-Type: application/json" \
  -d '{"token":"TOKEN","action":"list_certs","unlinked":true}'
```
可選：`from_number`、`person_type`+`person_id`、`unlinked`（只顯示未關聯）。

### link_cert — 關聯憑證到人員
```bash
curl -s -X POST "https://fotan.techforliving.net/api/skill" \
  -H "Content-Type: application/json" \
  -d '{"token":"TOKEN","action":"link_cert","cert_id":5,"person_type":"guest","person_id":23,"person_name":"陳大文"}'
```
自動填補 attendance.payment_method = 'receipt_uploaded'（如空白）。

### unlink_cert — 取消關聯
```bash
curl -s -X POST "https://fotan.techforliving.net/api/skill" \
  -H "Content-Type: application/json" \
  -d '{"token":"TOKEN","action":"unlink_cert","cert_id":5}'
```
