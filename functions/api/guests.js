export async function onRequest(context) {
  ...
  try {
    ...
    if (request.method === 'POST') {
      ...
      const { name, professional, tel, invited_by } = body;
      if (!name) return Response.json({ error: 'Name required' }, { status: 400, headers: cors });
      const result = await env.DB.prepare('INSERT INTO guests (name, professional, tel, invited_by) VALUES (?,?,?,?)')
        .bind(name, professional || '', tel || '', invited_by || '').run();
    ...
    if (request.method === 'PUT') {
      ...
      const body = await request.json();
      const sets = [], vals = [];
      for (const k of ['name','professional','tel','invited_by','active']) {
