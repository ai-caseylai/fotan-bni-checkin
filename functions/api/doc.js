  ...
  const doc = {
    ...
    endpoints: {
      ...
      '/api/attendance': {
        ...
        post: { body: '{meeting_id,person_type,person_id,payment,arrival_time,remark,seat_order}' },
        put: { body: '{id,payment,arrival_time,table_number,seat_order,price_tier,substitute}' },
      },
      '/api/members': {
        methods: 'GET,POST,PUT,DELETE',
        auth: 'Cookie',
        get: { params: '?id=X 或 ?all=1（含非活躍）' },
        post: { body: '{name,tel,email,professional,role,fee_paid_date,table_number,seat_order,bio}' },
        put: { body: '{id,name,tel,email,professional,role,table_number,seat_order,bio,active,tags}' },
      },
      '/api/guests': {
        methods: 'GET,POST,PUT,DELETE',
        auth: 'Cookie',
        ...
        put: { body: '{id,name,tel,professional,invited_by,meeting_id,vip,table_number,seat_order}' },
      },
      '/api/auth': {
        methods: 'GET,POST',
      ...
      '/api/settings': {
        methods: 'GET,PUT',
        auth: 'Cookie',
        description: 'Key-value 系統設定（標題、付款連結、QR、密碼、枱名 seating_names_{meeting_id} 等）',
      },
      '/api/stats': {
        methods: 'GET',
        auth: 'Cookie',
